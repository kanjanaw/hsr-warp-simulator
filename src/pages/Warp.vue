...
import { FIVE_STAR_LIGHT_CONES, MASTER_DATA_VERSION } from '../data/masterData.js'
...
].map(([id, name, icon]) => ({ id, name, kind: 'ตัวละคร', debutVersion: FOUR_STAR_DEBUT_VERSIONS[name] || '1.0', image: `https://raw.githubusercontent.com/Mar-7th/StarRailRes/master/icon/character/${icon}.png` }))
  ...
  <main class="warp-page" :style="{ '--banner-image': `url(${banner.image})` }">
      ...
      <Transition name="warp-animation-fade">
        <div v-if="isWarpAnimating" :class="['warp-animation', `phase-${warpAnimationPhase}`]" aria-live="polite">
          ...
          <div class="warp-animation-label">
            <strong>{{ warpAnimationPhase === 'charging' ? 'กำลังเริ่มการวาร์ป' : warpAnimationPhase === 'five-star' ? 'ตรวจพบสัญญาณระดับ 5★' : warpAnimationPhase === 'four-star' ? 'ตรวจพบสัญญาณระดับ 4★' : 'กำลังเปิดเผยผลลัพธ์' }}</strong>
    ...
    <div v-if="showResults" class="modal-backdrop" @click.self="showResults = false">
      <section class="result-modal">
        <button class="close" @click="showResults = false">×</button>
        ...
        <div :class="['result-grid', { single: latestResults.length === 1 }]">
          <article v-for="(item, index) in latestResults" :key="`${item.createdAt}-${index}`" :class="['result-card', `rarity-${item.rarity}`]" :style="{ '--delay': `${index * 55}ms` }">
        ...
        <button class="continue" @click="showResults = false">แตะเพื่อดำเนินการต่อ</button>
      </section>
    </div>
